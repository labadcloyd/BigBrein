...
import EditableQuiz from './editableQuiz'
...
export default function CreateNote(props){
	...
	/* controlling the title of the quiz */
	function quizNameChange(event){
	...
	async function postData(){
		try{
			...
			if(!quizData){
				const response = 'Missing Input: No qestions and answers added'
				setResponse(response)
				setResponseAvailable(true)
				return
			}
			if(!folderID){
				const response = 'Missing Input: Please select a folder to save the quiz set'
				setResponse(response)
				setResponseAvailable(true)
				return
			}
			...
			const response = await axios.post('/api/note', {title: quizTitle, quizData:quizData, folderID: folderID, username:username})
					...
					<div className={css.flashcardComponentContainer}>
						...
						<div className={css.optionWrapperWrapper}>
							...
							<div className={css.optionWrapper} style={{display:showSaveOptions?'flex':'none'}}>
								<input placeholder='Title for Quiz' value={quizTitle} onChange={quizNameChange} maxLength="50" required />
