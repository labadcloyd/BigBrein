...
export default function EditFlashcard(props){
	...
	const {session, userFolders, currentNoteSet, currentNoteTitle, currentNoteID, folderQuery, username} = props
	...
	const [noteData, setNoteData] = useState(currentNoteSet)
	...
	async function postData(){
		try{
			...
			const response = await axios.patch('/api/note', {title: noteTitle, noteData:noteData, folderID: folderID, username:username})
			...
			<div className={css.folderWrapperFiles}>
				<div className={css.flashcardComponentWrapper}>
					<div className={css.flashcardComponentContainer}>
						...
						<EditableNote currentNote={currentNoteSet} getData={getData} />
					</div>
				</div>
			</div>
		</FolderWrapper>
